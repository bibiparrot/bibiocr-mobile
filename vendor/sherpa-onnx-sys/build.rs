...
fn try_main() -> Result<(), DynError> {
    ...
    if link_mode == LinkMode::Shared && matches!(target_os.as_str(), "linux" | "macos" | "ios" | "android") {
        ...
        // Android APK packaging copies these libraries from SHERPA_ONNX_LIB_DIR.
        // Copying them into target/release is redundant and fails under Windows CI
        // when the scanner briefly locks a just-extracted .so (os error 32).
