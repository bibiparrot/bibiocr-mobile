        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        match event {
            ...
            InputEvent::KeyEvent(key) => {
                match key.key_code() {
                    ...
                    keycode => {
                        ...
                        // GameActivity also sends committed text separately. Without this guard
                        // Latin letters are inserted twice while a software IME is open.
                        if IME_ALLOWED.load(Ordering::Relaxed)
                            && key.device_id() <= 0
            ...
            InputEvent::TextEvent(state) => {
                if !IME_ALLOWED.load(Ordering::Relaxed) || state.text.is_empty() {
                    return input_status;
                }
                let ime = if state.compose_region.is_some() {
                    event::Ime::Preedit(state.text.clone(), None)
                } else {
                    event::Ime::Commit(state.text.clone())
                };
                callback(
                    event::Event::WindowEvent {
                        window_id: window::WindowId(WindowId),
                ...
                if state.compose_region.is_none() {
